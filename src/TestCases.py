...
# A basic valid certificate, signed directly by the root CA, that contains its
# fqdn in the CN
...
class InvalidNameNull(TestCaseImmed):
    ...
    def preSign(self, asnObj):
        asnObj.getComponentByPosition(0).getComponentByName('subject').\
               getComponentByPosition(0).getComponentByPosition(5).\
               getComponentByPosition(0).setComponentByName('value', \
                rfc2459.TeletexCommonName(getInvalidNullDomain(self.fqdn).\
                                          encode('utf-8')));
        return asnObj;

# A basic certificate that contains a NotBefore time after the current time
...
# A basic certificate that contains a NotAfter time before the current time
...
class InvalidExtendedKeyUsage(TestCaseImmed):
    def __init__(self, fqdn, info):
        ...
        self.getServCert().addExtension(ExtendedKeyUsage(serverAuth=False, \
                 clientAuth=True, codeSigning=True, emailProtection=True,\
                 timeStamping=True));
...
class InvalidSelfSign(TestCaseImmed):
    def __init__(self, fqdn, info):
        ...
        self.getServCert().signer = CertSign(None, \
...
# A chain of certificates where the first intermediate CA is missing a basic
...
# A chain of certificates where the first intermediate CA has keyCertSign
# marked as true in the key usage extension but lacked the basic constraint
# extension entirely
...
class InvalidIntCALoop(TestCaseChained):
    def __init__(self, fqdn, info):
        ...
        self.getFirstCA().signer = CertSign(None, \
